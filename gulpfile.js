...
gulp.task('tes', function() {
  log('-> ' + images.in + ", " + images.out);
});

gulp.task('compass', function() {
  log('-> Compile Compass Styles');
  return gulp.src(styles.in)
    .pipe($.plumber({
        errorHandler: function(err) {
          console.log(err);
    ...
    .pipe($.compass({
        ...
        sass: 'sass',
        image: 'assets/img',
        generated_images_path: '../build/assets/img'
    }))
    // .pipe($.size({ title: 'styles In Size' }))
    // .pipe($.pleeease(styles.pleeeaseOpt))
    // .pipe($.size({ title: 'styles Out Size' }))
...
// Build Task
gulp.task('build', ['compass', 'jade', 'html', 'js', 'images', 'vendors', 'favicon']);
...
gulp.task('help', function () {
  ...
  console.log('');
  console.log("ARRIIBBBAAAAAAAAAA" + views.out);
});
